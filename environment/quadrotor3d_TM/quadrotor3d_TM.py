...
class Quadrotor3D_TM(gym.Env):
    ...
    # metadata['Q'] = np.diag([10, 10, 10, 1, 1, 1, 1, 1, 1, 1, 1, 1])
    # metadata['R'] = np.diag([10, 1e4, 1e4, 1e4])
    ...
    metadata['kM'] = 7.94e-12
    metadata['omega_max'] = 37267.8 * 0.5  # 削减最大推力为2.0 * (0.5 ** 2) = 0.5
    metadata['Q'] = np.diag([50, 50, 50, 1, 1, 1, 10, 10, 10, 10, 10, 10])
    metadata['R'] = np.diag([1, 1, 1, 1])
    ...
    def __init__(self, options={}, theoretic_mode=False):
        ...
        self.state_low = np.array([-1.5, -1.5, -1.5, -10, -10, -10, -np.pi, -np.pi/2, -np.pi, -10, -10, -10])
        self.state_high = np.array([1.5, 1.5, 1.5, 10, 10, 10, np.pi, np.pi/2, np.pi, 10, 10, 10])
        self.observation_space = spaces.Box(low=self.state_low, high=self.state_high, shape=(self.metadata['nx'],), dtype=np.float64)
        # 动作空间: 4个电机推力
        # tau_max = 7e-3
        # self.u_min = np.array([0.0, -tau_max, -tau_max, -tau_max])
        # self.u_max = np.array([1.0, tau_max, tau_max, tau_max])
        tau_max = 2
        self.u_min = np.array([0.0, -tau_max, -tau_max, -tau_max])
        self.u_max = np.array([self.metadata['m']*self.metadata['g']*2, tau_max, tau_max, tau_max])
    ...
    def step(self, action):
        ...
        reward = -(self.state.T @ self.Q @ self.state + delta_action.T @ self.R @ delta_action) * self.dt + 1000 * self.dt * (1 - self.theoretic_mode)
        if np.all(np.abs(self.state) < np.array([0.01]*12)):
            terminated = True
            reward += 1000 * (1 - self.theoretic_mode)
        else:
            terminated = False
        if not self.observation_space.contains(self.state):
            crashed = True
            reward -= 500 * (1 - self.theoretic_mode)
...
def linearize_at_origin(acados_model):
    ...
    u0 = np.array([acados_model.metadata['m'] * acados_model.metadata['g'], 0, 0, 0])
