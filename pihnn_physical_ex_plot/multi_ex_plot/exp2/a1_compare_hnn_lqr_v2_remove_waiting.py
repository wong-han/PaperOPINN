...
def label_from_state_file(state_file):
    suffix = state_file.name.replace("state_data_", "").replace(".txt", "")
    return suffix[-10:] if len(suffix) > 10 else suffix

def find_matching_u_file(state_file, data_dir, controller_name):
    suffix = state_file.name.replace("state_data_", "").replace(".txt", "")
    candidates = [data_dir / f"u_data_{suffix}.txt"]

    controller_lower = controller_name.lower()
    controller_suffix = f"_{controller_lower}"
    if suffix.endswith(controller_suffix):
        suffix_without_controller = suffix[: -len(controller_suffix)]
        candidates.append(data_dir / f"u_data_{suffix_without_controller}.txt")
    else:
        candidates.append(data_dir / f"u_data_{suffix}_{controller_lower}.txt")
...
def load_state_after_control_start(state_file, data_dir):
    state_data = load_data(state_file)
    suffix = state_file.name.replace("state_data_", "").replace(".txt", "")
    controller_name = "HNN" if "hnn" in data_dir.name.lower() else "LQR"
    u_file = find_matching_u_file(state_file, data_dir, controller_name)
    if u_file is not None:
        u_data = load_data(u_file)
        state_data = state_data[state_data[:, 0] >= u_data[0, 0]]
    return state_data

def plot_all_trajectories_for_controller(data_dir, controller_name, output_path, base_color):
    state_files = sorted(data_dir.glob("state_data_*.txt"))
    if not state_files:
        print(f"No state_data_*.txt files found in {data_dir}, skipping {controller_name} summary.")
        return None

    print(f"Generating all {controller_name} trajectories summary...")
    fig = plt.figure(figsize=(11, 8))
    ax = fig.add_subplot(111, projection="3d")
    colors = plt.cm.viridis(np.linspace(0.05, 0.95, len(state_files)))

    plotted = 0
    all_x, all_y, all_z = [], [], []
    for idx, state_file in enumerate(state_files):
        state_data = load_state_after_control_start(state_file, data_dir)
        if state_data.shape[0] < 2:
            print(f"Warning: {state_file.name} has fewer than 2 valid samples, skipped.")
            continue

        x, y, z = state_data[:, 1], state_data[:, 2], state_data[:, 3]
        label = label_from_state_file(state_file)
        color = tuple(colors[idx]) if len(state_files) > 1 else base_color
        print(f"  Plotting {controller_name}: {state_file.name}")
        ax.plot(x, y, z, color=color, linewidth=1.8, label=label)
        ax.scatter(x[0], y[0], z[0], color=color, s=35, marker="o", edgecolors="black", linewidths=0.4)
        ax.scatter(x[-1], y[-1], z[-1], color=color, s=45, marker="x")
        all_x.append(x)
        all_y.append(y)
        all_z.append(z)
        plotted += 1
    ...
    if plotted == 0:
        plt.close(fig)
        print(f"Warning: no valid {controller_name} trajectories were plotted.")
        return None

    ax.scatter(0.0, 0.0, 0.9, color="blue", s=120, marker="*", edgecolors="black", label="Target")
    ax.set_xlabel("X (m)")
    ax.set_ylabel("Y (m)")
    ax.set_zlabel("Z (m)")
    ax.set_title(f"All {controller_name} 3D Trajectories", fontsize=14, pad=20)
    ax.legend(ncol=2, fontsize=8, title="Run")
    ax.grid(True, linestyle=":", alpha=0.5)

    x_all = np.concatenate(all_x)
    y_all = np.concatenate(all_y)
    z_all = np.concatenate(all_z)
    x_mid = 0.5 * (np.min(x_all) + np.max(x_all))
    y_mid = 0.5 * (np.min(y_all) + np.max(y_all))
    z_mid = 0.5 * (np.min(z_all) + np.max(z_all))
    span = max(np.ptp(x_all), np.ptp(y_all), np.ptp(z_all), 0.1) * 0.55
    ...
    print(f"Saved all {controller_name} trajectories summary to: {output_path}")
    return output_path
...
def main():
    ...
    hnn_state_file = hnn_files[1] if len(hnn_files) > 8 else hnn_files[-1]
    ...
    lqr_state_file = lqr_files[1] if len(lqr_files) > 6 else lqr_files[-1]
    
    # Load state data
    hnn_state_data = load_data(hnn_state_file)
    lqr_state_data = load_data(lqr_state_file)
    
    # Load control data to determine control start times
    ...
    hnn_u_data_preloaded = None
    lqr_u_data_preloaded = None
    
    if hnn_u_file.exists() and lqr_u_file.exists():
        hnn_u_data_preloaded = load_data(hnn_u_file)
        lqr_u_data_preloaded = load_data(lqr_u_file)
        
        # Filter state data to only keep timestamps >= control start time
        hnn_state_data = hnn_state_data[hnn_state_data[:, 0] >= hnn_u_data_preloaded[0, 0]]
        lqr_state_data = lqr_state_data[lqr_state_data[:, 0] >= lqr_u_data_preloaded[0, 0]]
    ...
    fig_3d_path = images_dir / "compare_trajectory_3d_rm_wait.png"
    ...
    fig_states_path = images_dir / "compare_states_rm_wait.png"
    ...
    if hnn_u_file.exists() and lqr_u_file.exists():
        ...
        fig_ctrl_path = images_dir / "compare_controls_rm_wait.png"
    ...
    # --- Figure 4/5: All trajectories by controller ---
    plot_all_trajectories_for_controller(
        hnn_dir,
        "HNN",
        images_dir / "all_hnn_trajectories_3d_rm_wait.png",
        color_hnn,
    )
    plot_all_trajectories_for_controller(
        lqr_dir,
        "LQR",
        images_dir / "all_lqr_trajectories_3d_rm_wait.png",
        color_lqr,
    )
    plt.show()
