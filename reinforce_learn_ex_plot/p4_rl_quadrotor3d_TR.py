...
def train_rl():
    ...
    model_path = os.path.join(model_dir, "rl_quadrotor3d.pth")
...
def simulate_once():
    ...
    rl_path = os.path.join("model", "rl_quadrotor3d.pth")
...
def monte_carlo_simulation():
    ...
    traj_u_rl = np.stack(all_traj_u_rl, axis=0)
    traj_x_rl = np.stack(all_traj_x_rl, axis=0)
    ...
    print(f"RL: mean={np.mean(returns_rl):.3f}, std={np.std(returns_rl):.3f}")
    if sp_loaded:
    ...
    min_val = min(np.min(returns_rl), np.min(returns_lqr))
    max_val = max(np.max(returns_rl), np.max(returns_lqr))
    if sp_loaded:
        min_val = min(min_val, np.min(returns_sp))
        max_val = max(max_val, np.max(returns_sp))
    if hnn_loaded:
        min_val = min(min_val, np.min(returns_hnn))
        max_val = max(max_val, np.max(returns_hnn))
    bins = np.linspace(min_val, max_val, 20)
    
    plt.hist(returns_rl, bins=bins, alpha=0.5, label="RL", color=colors["RL"])
    if sp_loaded:
