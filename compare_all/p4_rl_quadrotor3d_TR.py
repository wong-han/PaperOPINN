...
def monte_carlo_simulation():
    ...
    if mpc_loaded:
        print(f"MPC: {np.mean(success_mask_mpc)*100:.1f}%")
    if sp_loaded:
    ...
    if mpc_loaded:
        data_returns.extend(-filtered_returns_mpc)
        data_controllers.extend(["MPC"] * len(filtered_returns_mpc))
    ...
    # 颜色: HNN用紫色，RL用蓝色，SPNet用绿色，MPC用红色，LQR用橙色
    order = ["LQR"]
    palette = ["#F39B7F"]  # Orange for LQR
    if mpc_loaded:
        order.append("MPC")
        palette.append("#E64B35")  # Red for MPC
    if sp_loaded:
        order.append("SPNet")
