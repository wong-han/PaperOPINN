...
class Args:
    ...
    filename_prefix = "sp_nonlinear2d_theoretical_"
...
class SPNet(nn.Module):
    def __init__(self, env):
        self.env = env
        super().__init__()
        self.net = SIREN(in_features=2, out_features=1, hidden_features=64, hidden_layers=2, norm_scale=3.0)
...
def train_sp():
    ...
    print("--- Phase 1: Data Generation using Theoretical Optimal Control ---")
    
    N_samples = 50000
    X_data = np.random.uniform(-3.0, 3.0, (N_samples, 2)).astype(np.float32)
    
    # Theoretical optimal control: u* = -(cos(2*x1) + 2) * x2
    x1 = X_data[:, 0]
    x2 = X_data[:, 1]
    U_data = (-(np.cos(2*x1) + 2) * x2).reshape(-1, 1).astype(np.float32)

    print(f"Generated {len(X_data)} samples instantly.")
    ...
    step = 0
    while time.time() - train_start < 60:
    ...
    model_path = os.path.join(model_dir, "sp_nonlinear2d_theoretical.pth")
...
def simulate_once():
    ...
    model = SPNet(env)
    model_path = "model/" + "sp_nonlinear2d_theoretical.pth"
    if os.path.exists(model_path):
        model.load_state_dict(torch.load(model_path, map_location=Args.device))
    else:
