...
def main():
    ...
    # Locate early data files
    hnn_state_file = hnn_dir / "state_data_adjusted.txt"
    hnn_u_file = hnn_dir / "u_data.txt"
    lqr_state_file = lqr_dir / "state_data_adjusted.txt"
    lqr_u_file = lqr_dir / "u_data.txt"
    
    if not (hnn_state_file.exists() and lqr_state_file.exists()):
        print("Error: Early state data files not found.")
        return
    ...
    # Process HNN states
    # Columns: 0:t, 1:x, 2:y, 3:z, 4:roll, 5:pitch, 6:yaw, 7:vx, 8:vy, 9:vz
    hnn_t = hnn_state_data[:, 0]
    hnn_t_rel = hnn_t - hnn_t[0]
    hnn_x, hnn_y, hnn_z = hnn_state_data[:, 1], hnn_state_data[:, 2], hnn_state_data[:, 3]
    hnn_phi, hnn_theta, hnn_psi = hnn_state_data[:, 4], hnn_state_data[:, 5], hnn_state_data[:, 6]
    hnn_vx, hnn_vy, hnn_vz = hnn_state_data[:, 7], hnn_state_data[:, 8], hnn_state_data[:, 9]
    
    # Process LQR states
    lqr_t = lqr_state_data[:, 0]
    lqr_t_rel = lqr_t - lqr_t[0]
    lqr_x, lqr_y, lqr_z = lqr_state_data[:, 1], lqr_state_data[:, 2], lqr_state_data[:, 3]
    lqr_phi, lqr_theta, lqr_psi = lqr_state_data[:, 4], lqr_state_data[:, 5], lqr_state_data[:, 6]
    lqr_vx, lqr_vy, lqr_vz = lqr_state_data[:, 7], lqr_state_data[:, 8], lqr_state_data[:, 9]
    
    # Color definition (Ours = Red, LQR = Green)
    color_hnn = '#d62728' # Red
    color_lqr = '#2ca02c' # Green
    
    # Target state for early data is [0.58, 0.0, 0.9]
    ...
    # Target state
    ax_3d.scatter(target_pos[0], target_pos[1], target_pos[2], color='blue', s=150, marker='*', edgecolors='black', label=f'Target {tuple(target_pos)}', zorder=6)
    
    ax_3d.set_xlabel('X (m)')
    ax_3d.set_ylabel('Y (m)')
    ax_3d.set_zlabel('Z (m)')
    ax_3d.set_title('3D Trajectory Comparison (Early Data): HNN (Ours) vs. LQR', fontsize=14, pad=20)
    ax_3d.legend()
    plt.tight_layout()
    
    fig_3d_path = plots_dir / "compare_trajectory_3d.png"
    ...
    axs[0, 0].axhline(y=target_pos[0], color='black', linestyle=':', alpha=0.7, label='Target')
    ...
    axs[0, 1].axhline(y=target_pos[1], color='black', linestyle=':', alpha=0.7)
    ...
    axs[0, 2].axhline(y=target_pos[2], color='black', linestyle=':', alpha=0.7)
    ...
    fig_states.suptitle('State Comparison (Early Data): HNN (Ours) vs. LQR', fontsize=16, y=0.98)
    plt.tight_layout()
    
    fig_states_path = plots_dir / "compare_states.png"
    ...
    if hnn_u_file.exists() and lqr_u_file.exists():
        ...
        # HNN control inputs - align relative to control start
        hnn_tu = hnn_u_data[:, 0]
        hnn_tu_rel = hnn_tu - hnn_tu[0]
        hnn_u1, hnn_u2, hnn_u3, hnn_u4 = hnn_u_data[:, 1], hnn_u_data[:, 2], hnn_u_data[:, 3], hnn_u_data[:, 4]
        
        # LQR control inputs - align relative to control start
        lqr_tu = lqr_u_data[:, 0]
        lqr_tu_rel = lqr_tu - lqr_tu[0]
        ...
        fig_control.suptitle('Control Inputs Comparison (Early Data): HNN (Ours) vs. LQR', fontsize=16, y=0.98)
        plt.tight_layout()
        
        fig_ctrl_path = plots_dir / "compare_controls.png"
