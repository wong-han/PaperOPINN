...
class Args:
    ...
    # SIREN 网络对学习率非常敏感，默认 1e-3 容易导致梯度爆炸或 Loss 震荡不降，改为 1e-4
    learning_rate = 1e-4
    device = "cuda" if torch.cuda.is_available() else "cpu"
    filename_prefix = "sp_quadrotor3d_constrained_"
    batch_size = 2048
...
class SPNet(nn.Module):
    def __init__(self, env):
        self.env = env
        super().__init__()
        # 控制策略函数通常比较平滑，SIREN 默认的 omega_0=30 频率太高容易拟合出剧烈震荡的无效策略
        # 我们把 omega_0 降为 10（甚至 5）以匹配控制任务的平滑特性，大幅提高拟合精度。
        self.net = SIREN(in_features=9, out_features=4, hidden_features=128, hidden_layers=3, norm_scale=1.0, omega_0=10)
...
def train_sp():
    ...
    #     init_state[1] = np.abs(init_state[1])
    ...
    #         except Exception:
    #             # 求解失败，直接放弃该回合整个轨迹，重新 reset
    #             valid_traj = False
    #             break 
                
    #         # 环境前向演化
    #         state, reward, terminated, truncated, info = env.step(u_opt)
            
    #         if info.get('data', {}).get('crashed', [False])[-1]:
    #             valid_traj = False
    #             break
    ...
    #     if valid_traj and len(x_traj) > 0:
    ...
    # np.save('data/X_trajs_constrained.npy', np.array(X_trajs, dtype=object), allow_pickle=True)
    # np.save('data/U_trajs_constrained.npy', np.array(U_trajs, dtype=object), allow_pickle=True)
    # print("Trajectories saved to data/X_trajs_constrained.npy and data/U_trajs_constrained.npy")
    ...
    # plt.savefig('image/mpc_expert_trajectories_3d_constrained.png')
    # print("Saved 3D trajectory plot to mpc_expert_trajectories_3d_constrained.png")
    # plt.close()
    ...
    # for traj in X_trajs:
    #     ax_x.plot(traj[:, 0], alpha=0.6)
    #     ax_y.plot(traj[:, 1], alpha=0.6)
    #     ax_z.plot(traj[:, 2], alpha=0.6)
    ...
    # ax_z.set_ylabel('Z')
    # plt.savefig('image/mpc_expert_trajectories_3d_constrained_time_curve.png')
    # print("Saved 3D trajectory plot to mpc_expert_trajectories_3d_constrained_time_curve.png")
    ...
    X_trajs_raw = np.load('data/X_trajs_constrained.npy', allow_pickle=True)
    U_trajs_raw = np.load('data/U_trajs_constrained.npy', allow_pickle=True)
    ...
    while True:
        ...
        if patience_counter >= patience:
            print(f"Loss maintained below 0.0005 for {patience} steps. Stopping training.")
            break
    ...
    model_path = os.path.join(model_dir, "sp_quadrotor3d_constrained.pth")
...
def monte_carlo_simulation():
    ...
    sp_path = os.path.join("model", "sp_quadrotor3d_constrained.pth")
    ...
    if hnn_loaded:
        palette = ["#f6c85f", "#ff9f43", "#88bde6"]  # LQR, SPNet, HNN
        data_to_plot_J = [-filtered_returns_lqr, -filtered_returns_sp, -filtered_returns_hnn]
        labels_J = [r"LQR", r"SPNet", r"HNN"]
    else:
        palette = ["#f6c85f", "#ff9f43"]
        data_to_plot_J = [-filtered_returns_lqr, -filtered_returns_sp]
        labels_J = [r"LQR", r"SPNet"]
    ...
    for i, y_data in enumerate(data_to_plot_J):
    ...
    if hnn_loaded:
        ...
        df_logymin = pd.DataFrame({
            r"$\log_{10}|y_{\mathrm{min}}|$": np.concatenate([log_abs_ymin_lqr, log_abs_ymin_sp, log_abs_ymin_hnn]),
            "Controller": ["LQR"] * len(log_abs_ymin_lqr) + ["SPNet"] * len(log_abs_ymin_sp) + ["HNN"] * len(log_abs_ymin_hnn)
        })
        violin_palette = ["#389826", "#e67e22", "#9558B2"]
    else:
        df_logymin = pd.DataFrame({
            r"$\log_{10}|y_{\mathrm{min}}|$": np.concatenate([log_abs_ymin_lqr, log_abs_ymin_sp]),
            "Controller": ["LQR"] * len(log_abs_ymin_lqr) + ["SPNet"] * len(log_abs_ymin_sp)
        })
        violin_palette = ["#389826", "#e67e22"]
    ...
    plt.savefig(os.path.join(image_dir, args.filename_prefix + "ymin_violinplot.svg"), dpi=600, bbox_inches='tight', pad_inches=0.5)
    plt.show()
